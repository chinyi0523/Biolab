...
class CTtP229TouchButton {
    struct CTtp229Prop {
        ...
        uint16_t
            IgnoreNextEvent : 1;  // When reading 16th key and if it is pressed,
                                  // SDO stays low for 2ms. If we enable
                                  // interrupt before that, then it will trigger
                                  // after 2ms, only to find the same condition.
                                  // To make things worse, at the end of reading
                                  // the pin will stay low and generate another
                                  // interrupt. TBD: One possible fix is to send
                                  // more pulses to make it roll over to HIGH.
                                  // Have to find out if all 16 keys can be
                                  // pressed in multi-key scenario (NOT
                                  // supported yet).
...
#if defined(INT0_PIN) || defined(INT1_PIN)
    // Detaching the interrupt after receiving the data can cause problem in
    // sleeping. If the interrupt is not properly dispatched, it can lead to
    // permanent sleep and can't wake up from button
    ...
    static void SetInterruptHandler() {
        ...
        if (INT0_PIN == g_prop.SdoPin) {
            ...
            attachInterrupt(
                0, HandleButtonEvent,
                RISING);  // The pin goes down for 93us and then raises that is
                          // when the device is ready (technically after 10us)
        }
#endif
   ...
   public:
    ...
    static uint16_t GetButtonStatus() {
        ...
        returnValue |=
            g_prop
                .UnhandledButtonPresses;  // and also include any data that was
                                          // received that we have not sent yet.
        g_prop.UnhandledButtonPresses = 0;
#endif
    ...
    static CTtp229ButtonEvent GetButtonEvent() {
        ...
        if (
            ...
            if (g_prop.HasPendingInterrupt) {
                RemoveInterruptHandler();  // From this point upto
                                           // SetInterruptHandler is called,
                                           // ensure there is no return path
                                           // that will leave without
                                           // SetInterruptHandler
            } else {
                DEBUG_BUTTON16(
                    Serial.println("GetButtonEvent: No interrupt pending"));
                return returnValue;
            }
        }
#endif
...
#if defined(INT0_PIN) || defined(INT1_PIN)
        currValue |=
            g_prop.UnhandledButtonPresses;  // Get any previously returned but
                                            // not returned now values also into
                                            // the mix
#endif
        ...
        if (0 != pressed) {
            ...
            g_prop.UnhandledButtonPresses =
                currValue &
                ~g_prop.PreviousButtonValue;  // clear unhandled for this bit,
                                              // just in case
#endif
        } else if (0 != released) {
        ...
        if (((!returnValue.IsButtonReleased ||
              (0 == pressed))  // We handle release but no pending press
             && ((buttonNumber & 0x80) ==
                 0))  // or more button changes are detected
            || (returnValue.ButtonNumber ==
                0))  // safety in case interrupt and data mismatch or code bug
        {
        ...
        g_prop.IgnoreNextEvent =
            digitalRead(g_prop.SdoPin) ==
            LOW;  // If the pin is still low at the end of reading, ignore next
                  // event which is for data finished raise
        DEBUG_BUTTON16(Serial.print(g_prop.IgnoreNextEvent
                                        ? "Ignoring next event\n\r"
                                        : "Not ignoring\n\r"));
...
#if defined(INT0_PIN) || defined(INT1_PIN)
            DEBUG_STATUS(
                Serial.println("Are you not using interrupt? Should never come "
                               "here for interrupt based system."));
#endif
        }
